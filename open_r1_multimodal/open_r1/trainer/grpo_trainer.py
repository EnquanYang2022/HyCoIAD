...
import warnings
from open_r1_multimodal.open_r1.vlm_modules.qwen2_5vl_module_HpyCo import init_weight
...
from open_r1_multimodal.open_r1.vlm_modules.vlm_module import VLMBaseModule
...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        original_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_id, **model_init_kwargs)
        # LoRA
        ...
        #     if any(keyword not in n for keyword in self.vision_modules_keywords):
        #         p.requires_grad = False
        # self.new_vision_modules_keywords = [
        #     "FacT_c", "FacT_p", "gamma_v", "gamma_q","linear_a_q", "linear_b_q", "linear_a_v", "linear_b_v","curvature","norm_scale","FacT_d1","FacT_d2"
        # ]
        # self.new_vision_modules_keywords = [
        #     "linear_a_q", "linear_b_q", "linear_a_v", "linear_b_v","norm_scale","FacT_c", "FacT_p","FacT_d1","FacT_d2","gamma","k_cur"
        # ]
        # self.new_vision_modules_keywords = [
        # ]
        ...
        #Only Lora
        self.new_vision_modules_keywords = [
            "FacT_c", "FacT_p", "gamma", "linear_a_q", "linear_b_q", "linear_a_v", "linear_b_v",
            "k_cur", "norm_scale", "FacT_d"
        ]


        # Only Adapter
        # self.new_vision_modules_keywords = [
        #     "adapter_down","adapter_up","adapter_layer_norm_before"
        # ]
        ...
        if freeze_vision_modules:
            ...
            for n, p in model.named_parameters():
                # 检查是否是视觉模块并且不属于新增模块
                if any(keyword in n for keyword in self.vision_modules_keywords) and not any(
                        new_keyword in n for new_keyword in self.new_vision_modules_keywords):
                    p.requires_grad = False
        ...
        # 调整 state_dict，确保原模型中的qkv权重加载到新的qkv.qkv结构 这是针对LORA结构的
        for key in original_state_dict:
            if 'attn.qkv.' in key:  # 查找所有attn.qkv权重
                # 重新命名原始权重，将其加载到新结构 (qkv.qkv)
                new_key = key.replace('attn.qkv.', 'attn.qkv.qkv.')  # 例如: 'attn.qkv.weight' -> 'attn.qkv.qkv.weight'
                if new_key in new_model_state_dict:
                    new_model_state_dict[new_key] = original_state_dict[key]  # 将原始权重加载到新的键名
        # 调整 state_dict，确保原模型中的mlp权重加载到新的mlp.mlp结构 这是针对Adapter结构的
        # for key in original_state_dict:
        #     if 'mlp.' in key:  # 查找所有attn.qkv权重
        #         # 重新命名原始权重，将其加载到新结构 (qkv.qkv)
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            # 根据 FacT_c, FacT_p, FacT_d 区分不同的参数
            IBC_parameters = []
            Hpy_parameters = []
            other_parameters = []
            ...
            for name, param in opt_model.named_parameters():
                if not param.requires_grad:
                    continue
                if "FacT_c" in name or "FacT_p" in name or "FacT_d" in name:
                # if "FacT_c" in name or "FacT_p" in name:
                    IBC_parameters.append((name, param))
                elif "k_cur" in name:
                    Hpy_parameters.append((name, param))
                else:
            ...
            # FacT_c, FacT_p, FacT_d with decay
            qu_params_with_decay = [
                param for name, param in IBC_parameters
                if name in decay_parameter_names
            ]
            if qu_params_with_decay:
                optimizer_grouped_parameters.append({
                    "params": qu_params_with_decay,
                    "lr": 5e-5,
                    "weight_decay": self.args.weight_decay,
                })

            # FacT_c, FacT_p, FacT_d no decay
            qu_params_no_decay = [
                param for name, param in IBC_parameters
                if name not in decay_parameter_names
            ]
            if qu_params_no_decay:
                optimizer_grouped_parameters.append({
                    "params": qu_params_no_decay,
                    "lr": 5e-5,  #
                    "weight_decay": 0.0,
                })

            # Hpy_params with decay
            Hpy_params_with_decay = [
                param for name, param in Hpy_parameters
                if name in decay_parameter_names
            ]

            if Hpy_params_with_decay:
                optimizer_grouped_parameters.append({
                    "params": Hpy_params_with_decay,
                    "lr": 5e-6,
                    "weight_decay": 0.0,
                })
            ...
            if Hpy_params_no_decay:
                optimizer_grouped_parameters.append({
                    "params": Hpy_params_no_decay,
                    "lr": 5e-6,
                    "weight_decay": 0.0,
                })

            # other_params with decay
            other_params_with_decay = [
                param for name, param in other_parameters
                if name in decay_parameter_names
            ]
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        prompts_text = self.vlm_module.prepare_prompt(self.processing_class, inputs) #apply_chat
        ...
        for x in inputs:
            if "image" in x:
                ...
                imgs = [PIL.Image.open(os.path.join(x['image_root'],p)) for p in self._get_key_from_inputs(x, "image_path")]
            else:
                imgs = []
